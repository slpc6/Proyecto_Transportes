import api from './api';
import type { DatosEscoriaEnvio, RespuestaAPI } from '../types/escoria';

export const envioService = {

  async enviarEscoria(datos: DatosEscoriaEnvio): Promise<RespuestaAPI> {
    try {
      const response = await api.post('/escoria', datos);
      return {
        success: true,
  ...
  validarDatosEscoria(datos: DatosEscoriaEnvio): { valido: boolean; mensaje: string } {
