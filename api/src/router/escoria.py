...
def escoria(request: DatosEscoriaEnvio):
    try:
        # Rutas de archivos usando la clase Path
        input_file = os.path.join(Path.INPUT, "ESCORIA.xlsx")
        output_file = os.path.join(Path.OUTPUT, "ESCORIA.xlsx")
        
        # Verificar que existe el archivo de entrada
        if not os.path.exists(input_file):
            raise HTTPException(status_code=404, detail="Archivo ESCORIA.xlsx no encontrado en input/")
        
        # Crear directorio output si no existe
        os.makedirs(Path.OUTPUT, exist_ok=True)
        
        # Leer el archivo Excel existente
        df_existente = pd.read_excel(input_file, header=1)  # Saltar la primera fila (título)
